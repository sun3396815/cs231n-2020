...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # W1 (D, H), X (N, D), b1 (H,), fc1 (N, H)
        fc1 = np.dot(X, W1) + b1
        # ReLU (N, H)
        relu1 = np.maximum(0, fc1)
        # W2 (H, C), b2 (C,), scores (N, C)
        ...
        C, H = W2.shape[1], W2.shape[0]

        # now softmax(N, C) is the gradient of the loss function
        dSoft = softmax
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            # set replace to False so that no element will be choose more than once
